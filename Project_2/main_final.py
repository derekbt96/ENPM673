...
class Lane_detector_video:
	def __init__(self):
		...
		self.final_turn = 0.0
		self.slope_left = 0.0
		self.slope_right = 0.0

		self.road = np.zeros((180,400),np.uint8)
		self.road_fin = np.zeros((180,400),np.uint8)
	...
	def spin(self,img):
		...
		print(round(self.turn,2),' ',round(self.turn_R,2),' ',round(self.turn_L,2), '', round(self.final_turn,2))
	...
	def get_lane_points(self,img):
		...
		if lines is not None:
			...
			if left_lines.shape[0] > 0:
				...
				cv2.line(road_og1,(int(xfin1/i),int(yfin1/i)),(int(xfin2/i),int(yfin2/i)),(255,0,0),4)

				if xfin2 != xfin1:
					self.slope_left = float((yfin2 - yfin1)/(xfin2 - xfin1))
			...
			if right_lines.shape[0] > 0:
				...
				if xfin2 != xfin1:
					self.slope_right = float((yfin2 - yfin1)/(xfin2 - xfin1))
		...
		self.road = road_og
		self.road_fin = road_og1

		self.final_turn = (self.slope_left + self.slope_right)/2
	...
	def color_lane(self,img):
		...
		if sign == -1:
			...
			text = "Turning Left!"
		elif abs(self.final_turn) < 1:
			text = "Going straight!"

		image = cv2.putText(img2, text, org, font,  1, (40,40,40), 2, cv2.LINE_AA) 
